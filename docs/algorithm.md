...
- New: a final portfolio-facing README.md, including a "Why this project" section covering the real-flight-data angle, the imbalance-evaluation methodology, and the fault-detection framing that generalizes beyond this one dataset. Professor-specific pitches (which of these angles lands hardest for which target) live in the individual outreach emails, not in the shared public README, so no single page lists all three targets side by side
